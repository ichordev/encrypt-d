...
if(keyBits == 128 || keyBits == 192 || keyBits == 256){
	...
	unittest{
		void doEncryptECB(size_t bits, ubyte[] keyCT)(const ubyte[aesBlockSize] b){
			const ubyte[keyCT.length] key = keyCT;
			ubyte[aesBlockSize] a = [0x6B,0xC1,0xBE,0xE2,0x2E,0x40,0x9F,0x96,0xE9,0x3D,0x7E,0x11,0x73,0x93,0x17,0x2A];
			auto aes = AES!bits(key);
			aes.encryptECB(a);
			assert(a == b);
		}
		doEncryptECB!(256, [
	...
	unittest{
		void doDecryptECB(size_t bits, ubyte[] keyCT)(ubyte[aesBlockSize] a){
			const ubyte[keyCT.length] key = keyCT;
			auto aes = AES!bits(key);
			aes.decryptECB(a);
