...
if(keyBits == 128 || keyBits == 192 || keyBits == 256){
	...
	auto encryptECB(ubyte[aesBlockSize] buf) const{
	...
	unittest{
		...
		auto b = aes.encryptECB(a);
		static if(keyBits == 256){
			assert(b == [0xf3,0xee,0xd1,0xbd,0xb5,0xd2,0xa0,0x3c,0x06,0x4b,0x5a,0x7e,0x3d,0xb1,0x81,0xf8]);
		}else static if(keyBits == 192){
			assert(b == [0xbd,0x33,0x4f,0x1d,0x6e,0x45,0xf2,0x5f,0xf7,0x12,0xa2,0x14,0x57,0x1f,0xa5,0xcc]);
		}else static if(keyBits == 128){
			assert(b == [0x3A,0xD7,0x7B,0xB4,0x0D,0x7A,0x36,0x60,0xA8,0x9E,0xCA,0xF3,0x24,0x66,0xEF,0x97]);
		}else static assert(0);
	}
	
	auto decryptECB(ubyte[aesBlockSize] buf) const{
	...
	unittest{
		...
		auto aes = AES!keyBits(testKey);
		auto b = aes.decryptECB(a);
		assert(b == [0x6B,0xC1,0xBE,0xE2,0x2E,0x40,0x9F,0x96,0xE9,0x3D,0x7E,0x11,0x73,0x93,0x17,0x2A]);
	}
